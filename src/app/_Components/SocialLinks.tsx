"use client"
import React, { useEffect, useState } from 'react'
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Button } from "@/components/ui/button"
import { Twitter, Linkedin, GithubIcon } from "lucide-react"
import { updateAnotherProfile } from '@/controllers/controller'
interface SocialLink {
   
    platform: string
    link: string
}
  

const SocialLinks = ({ profileInfo, userId }:any) => {
    const [links, setLinks] = useState<SocialLink[]>([])
    const [newLink, setNewLink] = useState<SocialLink>({ platform: "", link: "" })
    const [loading, setloading] = useState<boolean>(false)
  useEffect(() => {
     
   
   setLinks(profileInfo?.sociallinks)
     
   }, [])
    const addLink = () => {
        if (newLink.platform && newLink.link) {
            setLinks([...links, { ...newLink }])
            setNewLink({ platform: "", link: "" })
        }
    }
  
    // const removeLink = (id: number) => {
    //     setLinks(links.filter((link) => link.id !== id))
    // }

    const changesSaved = async() => {
        try {
              
          if (newLink.platform && newLink.link) {
            setloading(true)
            
            const changeProcess = await updateAnotherProfile(userId, newLink  ,"sociallinks")
            if (changeProcess.success == true) {
              setLinks(changeProcess.data[0].sociallinks)
            }
          
            
          }
        } catch (error) {
          console.log("something went wrong")
          
        } finally {
          setloading(false)
        }
      
       
      }
  
    const getPlatformIcon = (platform: string) => {
        switch (platform.toLowerCase()) {
            case "github":
                return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-github"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg>
            case "twitter":
                return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-twitter"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"/></svg>
            case "linkedin":
                return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-linkedin"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
            default:
                return null
        }
    }
    return (
        <>
            <section id="social-links" className="mb-8 w-full">
                <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-white">Social Links</h2>
                <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <Label htmlFor="platform">Platform</Label>
                            <Input
                                id="platform"
                                value={newLink.platform}
                                onChange={(e) => setNewLink({ ...newLink, platform: e.target.value })}
                                placeholder="GitHub, Twitter, LinkedIn, etc."
                            />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="url">URL</Label>
                            <Input
                                id="url"
                                value={newLink.link}
                                onChange={(e) => setNewLink({ ...newLink, link: e.target.value })}
                                placeholder="https://platform.com/yourusername"
                            />
                        </div>
                    </div>
                    <Button onClick={changesSaved}>Add Social Link</Button>
                </div>
                 <div className="mt-6 space-y-2">
                    {links.map((items:any,idx:number) => (
                        <div key={idx} className="flex items-center space-x-2">
                            {getPlatformIcon(items.platform)}
                            <a href={items.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                                {items.platform}
                            </a>
                             {/* <Button variant="destructive" size="sm" onClick={() => removeLink(link.id)}>
                                Remove
                            </Button>  */}
                        </div> 
                     ))} 
                 </div> 
            </section>
        </>
    )
}

export default SocialLinks
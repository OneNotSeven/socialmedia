...
export async function POST(req: Request): Promise<NextResponse> {
    try {
        ...
        const following = await EcomSchema.updateOne({ _id: authuser }, { $push: { following: userId } })
        const followers = await EcomSchema.updateOne({ _id: userId }, { $push: { followers: authuser } })
